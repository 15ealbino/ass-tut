...
Two layers, mirroring test_memory_traffic.py / test_register_footprint.py:
  * Pure-function unit tests for `classify_branch_sense` / `analyze_branch_senses` — no gcc.
...
from app.compile import (
    analyze_branch_senses,
    analyze_cost,
    classify_branch_sense,
)
...
# ─── Unit: classify_branch_sense (pure, no gcc) ────────────────────────────────────
...
def test_classify_branch_sense(mnemonic, expected):
    assert classify_branch_sense(mnemonic) == expected
...
# ─── Unit: analyze_branch_senses (pure, no gcc) ───────────────────────────────────

def test_analyze_branch_senses_counts_per_line_and_totals():
    ...
    summary = analyze_branch_senses(line_map, asm_lines)

    assert line_map[1]["branch_sense_counts"] == {"signed": 1, "unconditional": 1}
    assert summary["branch_totals"] == {"signed": 1, "unconditional": 1}


def test_analyze_branch_senses_zero_entries_omitted_per_line():
    ...
    assert line_map[1]["branch_sense_counts"] == {}
    assert summary["branch_totals"] == {}


def test_analyze_branch_senses_orders_senses_for_stable_display():
    ...
    line_map = {1: {"c_lines": [1], "asm_lines": [1, 2, 3], "color": "#000"}}
    analyze_branch_senses(line_map, asm_lines)
    assert list(line_map[1]["branch_sense_counts"].keys()) == [
        "signed", "equality", "unconditional",
    ]


def test_analyze_branch_senses_totals_sum_across_lines():
    asm_lines = [
        "jl .L1",     # line 1: signed
...
def test_analyze_branch_senses_detects_unsigned_branch():
    ...
    line_map = {1: {"c_lines": [1], "asm_lines": [1, 2], "color": "#000"}}
    summary = analyze_branch_senses(line_map, asm_lines)
    assert line_map[1]["branch_sense_counts"] == {"unsigned": 2}
    assert summary["branch_totals"] == {"unsigned": 2}


def test_analyze_branch_senses_ignores_out_of_range_asm_lines():
    # Defensive: a stray asm index past the end is skipped, mirroring analyze_cost.
    asm_lines = ["jl .L1"]
    line_map = {1: {"c_lines": [1], "asm_lines": [1, 99], "color": "#000"}}
    summary = analyze_branch_senses(line_map, asm_lines)
    assert line_map[1]["branch_sense_counts"] == {"signed": 1}
    assert summary["branch_totals"] == {"signed": 1}


def test_analyze_branch_senses_empty_line_map():
    summary = analyze_branch_senses({}, [])
    assert summary["branch_totals"] == {}


def test_analyze_branch_senses_does_not_disturb_cost_fields():
    # Regression: running the branch pass after analyze_cost leaves the existing
    # cost/mix annotations intact and merely adds `branch_sense_counts`. Note the jump
...
async def test_compile_response_includes_branch_summary(client):
    ...
    body = r.json()
    assert body["branch_sense_summary"] is not None
    assert "branch_totals" in body["branch_sense_summary"]
    # Every mapped line carries a branch_sense_counts map (possibly empty).
    for mapping in body["line_map"].values():
        assert "branch_sense_counts" in mapping
        assert isinstance(mapping["branch_sense_counts"], dict)
...
async def test_compile_loop_emits_signed_branches(client):
    ...
    assert r.status_code == 200
    totals = r.json()["branch_sense_summary"]["branch_totals"]
